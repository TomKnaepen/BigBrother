            ...
            args = {
                events = {
                    ...
                    args = {
                        ...
                        reportself = {
                            name = L["Report Self"],
                            desc = L["Report events caused by Self"],
                            type = 'toggle',
                            get = function() return addon.db.profile.ReportSelf end,
                            set = function(v)
                                addon.db.profile.ReportSelf = v
                            end,
                            map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }
                        },
                        reporttanks = {
                ...
                checks = {
                    ...
                    args = {
                        ...
                        combat = {
                            ...
                            desc = L["Combat Buffs"],
                            type = 'toggle',
                            get = function() return addon.db.profile.CheckCombatBuffs end,
                            set = function(v) addon.db.profile.CheckCombatBuffs = v end,
                            map = { [false] = "|cffff4040Disabled|r", [true] = "|cff40ff40Enabled|r" }
                        },
                    },
                },
...
function addon:BroadcastVersion(force)
    ...
    if IsInGroup(LE_PARTY_CATEGORY_INSTANCE) then
        C_ChatInfo.SendAddonMessage(addon.name, msg, "INSTANCE_CHAT")
    elseif IsInRaid() then
        C_ChatInfo.SendAddonMessage(addon.name, msg, "RAID")
    elseif GetNumGroupMembers() ~= 0 then
        C_ChatInfo.SendAddonMessage(addon.name, msg, "PARTY")
    end
    if IsInGuild() then
        C_ChatInfo.SendAddonMessage(addon.name, msg, "GUILD")
    end
end

C_ChatInfo.RegisterAddonMessagePrefix(addon.name)
...
function addon:ConsumableCheck(Where, Full)
    ...
    local MissingAllCombatBuffList = {}

    local MissingIntBuffList = {}
    local MissingAPBuffList = {}
    local MissingStaBuffList = {}

    if not (self.db.profile.CheckFlasks or self.db.profile.CheckElixirs or self.db.profile.CheckFood or self.db.profile.CheckCombatBuffs) then
    ...
    for unit in RL:IterateRoster(false) do
        if self.db.profile.Groups[unit.subgroup] then
            ...
            -- table.insert(MissingCombatBuffList,unit.name)
        end
    end
    ...
    local buffNum = 0
    local attackpower = false
    local intellect = false
    local stamina = false
    if self.db.profile.CheckCombatBuffs then
        --now figure out who has less than 3 buffs
        for unit in RL:IterateRoster(false) do
            if self.db.profile.Groups[unit.subgroup] then
                -- check for missing combat buffs here
                buffNum = 0
                attackpower = false
                intellect = false
                stamina = false
                ...
                for i = 1, 40 do
                    ...
                    local _, _, _, _, _, _, _, _, _, spellId = UnitBuff(unit.unitid, i)

                    for i, v in ipairs(vars.SpellData.CombatBuffSplit.attackpower) do
                        if attackpower == true then
                            break
                        end
                    ...
                    for i, v in ipairs(vars.SpellData.CombatBuffSplit.intellect) do
                    ...
                    for i, v in ipairs(vars.SpellData.CombatBuffSplit.stamina) do
                ...
                if buffNum == 0 then
                    table.insert(MissingAllCombatBuffList, unit.name)
                else
                    if attackpower == false then
                        table.insert(MissingAPBuffList, unit.name)
                    end

                    if intellect == false then
                        table.insert(MissingIntBuffList, unit.name)
                    end

                    if stamina == false then
                        table.insert(MissingStaBuffList, unit.name)
                    end
                end
            end
        end
...
function addon:COMBAT_LOG_EVENT_UNFILTERED()
    ...
    elseif PolyBreak and is_playersrc and is_hostiledst
        ...
        and ccSpellNames[spellname] then
            ...
            elseif not is_playersrc and addon.db.profile.ReportSelf ~= true then -- rest is all direct player actions (if disabled)
                return
